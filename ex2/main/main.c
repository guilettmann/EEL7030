...
void app_main(){
    // Criação da estrutura para o pino de interrupção
    gpio_config_t io_conf;

    // Definição dos parâmetros do pino e da interrupção
    ...
    // Atribuição das definições anteriores
    gpio_config(&io_conf);

    // Instalação do serviço de interrupções pelo GPIO
    gpio_install_isr_service(ESP_INTR_FLAG_IRAM);

    // Adiciona a função de tratamento à interrupçao no pino
    gpio_isr_handler_add(INTERRUPT_PIN, gpio_isr_handler, NULL);
    
    char str[] = "Inverter";
    // Calcula o tamanho da string (-1 pra eliminar o caractere nulo: \0)
    int tamanho_str = sizeof(str)/sizeof(str[0]) - 1;
    ...
    while(1){
      ...
      if (interrupt){
        // O último elemento da string está na posição tamanho - 1 (pois começa a contar no elemento zero)
        inverter_string(str, 0, tamanho_str - 1);
        interrupt = 0;
      }
    };
}

void inverter_string(char str[], int a, int b) {
    /*
      O princípio de funcionamento dessa função é inverter os caracteres nas posições
      [a] e [b] da string. Iniciamos invertendo os dois caracteres limites (primeiro e último). Após
      converter, podemos fazer uma chamada recursiva para inverter os próximos (segundo e penúltimo)
      e assim suscessivamente.
      
      Sabemos que a string está completamente invertida quando os caracteres
      que devem ser invertidos tem o mesmo valor [a = b] ou quando o caractere "inicial" é maior que o
      "final" (a > b). Nesse caso, chamamos a condição de base, que encerra a recursividade.
    */

    // Declaração da condição base (saída da recursividade)
    if (a >= b) {
        return;
    }

    /*
      Como vamos inverter os dois caracteres, criamos uma variável temporária para armazenar um dos
      caracteres que serão invertidos. Nesse caso, estamos salvando na variável temporária o valor do caractere
      na posição menor (a), em seguida, substituítmos esse caractere pelo caractere na posição maior. Por fim,
      precisamos atualizar o valor na posição maior, copiando o valor que foi salvo na variável temporária.
    */
    char temp = str[a];
    str[a] = str[b];
    str[b] = temp;

    // Uma vez a inversão realizada, chamamos a inversão dos próximos dois caracteres de maneira recursiva    
    inverter_string(str, a + 1, b - 1);
};
