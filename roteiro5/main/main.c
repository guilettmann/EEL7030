...
void app_main(){
   ...
   unsigned char nome[] = "EEL7030 - MIC"; // cada elemento de uma string eh um byte codificado em ASCII que ocupa uma posição do array
...
void LCD_escreve_strings(unsigned char str1[], unsigned char str2[]){
   LCD_escreve_comando(0x80);//mover o cursor para a primeira linha (primeira pos)
   LCD_escreve_string(str1);
   LCD_escreve_comando(0xC0); // mover o cursor para a segunda linha (primeira pos)
...
void LCD_escreve_string(unsigned char str[]){//Escreva uma função que receba uma string (vetor de dados do tipo unsigned char
// codificados como ASCII) e escreva essa string no LCD. Caso a string possua mais de
// 16 caracteres, apenas os 16 primeiros devem ser escritos. Caso a string possua menos
// de 16 caracteres, as posições restantes do LCD devem ser apagadas (escrever
// espaços)
   int i = 0;
